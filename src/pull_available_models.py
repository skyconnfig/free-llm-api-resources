...
"""
免费 LLM API 资源获取脚本
从各个 AI 服务提供商获取可用的免费模型列表，并生成 README.md 文档
"""
...
from data import (
    MODEL_TO_NAME_MAPPING,      # 模型 ID 到显示名称的映射
    HYPERBOLIC_IGNORED_MODELS,  # Hyperbolic 平台需要忽略的模型列表
    LAMBDA_IGNORED_MODELS,      # Lambda Labs 平台需要忽略的模型列表
    OPENROUTER_IGNORED_MODELS,  # OpenRouter 平台需要忽略的模型列表
)
...
# 记录缺失的模型 ID（未在 MODEL_TO_NAME_MAPPING 中找到的模型）
MISSING_MODELS = set()
...
def get_groq_limits_for_stt_model(model_id, logger):
    """
    获取 Groq 语音转文本（STT）模型的限流信息
    
    参数:
        model_id: 模型 ID（如 whisper-large-v3）
        logger: 日志记录器
    
    返回:
        包含限流信息的字典，如 {"audio-seconds/minute": xxx, "requests/day": xxx}
    """
...
def get_groq_limits_for_model(model_id, script_dir, logger):
    ...
    参数:
        model_id: 模型 ID
        script_dir: 脚本目录（用于定位音频文件）
        logger: 日志记录器
    
    返回:
        包含限流信息的字典，对于 TTS 模型返回 None
    """
    # 如果是语音识别模型，使用专门的 STT 方法
...
def fetch_groq_models(logger):
    ...
    try:
        ...
        logger.error("Skipping Groq models.")
        return []
    except Exception as e:
...
def fetch_openrouter_models(logger):
    """
    获取 OpenRouter 平台的免费模型列表
    API: https://openrouter.ai/api/v1/models
    
    筛选条件:
    - 价格为免费的模型（pricing 为 0）
    - ID 包含 :free 后缀
    - 不在忽略列表中
    ...
    返回:
        免费模型列表，每个模型包含 id、name 和 limits
    """
...
def fetch_cloudflare_models(logger):
    """
    获取 Cloudflare Workers AI 平台的文本生成模型
    ...
    try:
        ...
        logger.error(f"Missing environment variable: {e}")
        logger.error("Skipping Cloudflare models.")
        return []
    except Exception as e:
        logger.error(f"Failed to fetch Cloudflare models: {e}")
        return []
...
def fetch_hyperbolic_models(logger):
    ...
    try:
        ...
        logger.error(f"Missing environment variable: {e}")
        logger.error("Skipping Hyperbolic models.")
        return []
    except Exception as e:
        logger.error(f"Failed to fetch Hyperbolic models: {e}")
        return []
...
def fetch_github_models(logger):
    ...
    返回:
        模型列表，每个模型包含 id 和 name（friendly_name）
    """
...
def fetch_gemini_limits(logger):
    """
    获取 Google AI Studio (Gemini) 的免费配额信息
    使用 Google Cloud Quotas API 查询配额
    ...
    try:
        ...
        logger.error(f"Failed to fetch Gemini limits: {e}")
        logger.error("Skipping Gemini quota information. Continue with empty limits.")
        return {}


def fetch_lambda_models(logger):
    """
    获取 Lambda Labs GPU 云平台的模型列表
    API: https://api.lambdalabs.com/v1/models
